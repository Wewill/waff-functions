...
// 	}
// 	if (!$current_edition)
// 		print "Oups¡ aucune edition en cours";
// 	unset($tmp_editions);
// 	$tmp_editions = null;
...
/**
	?? Utilisé en tant que fct dans views ??
*/
...
function func_get_formats($atts, $content='') {
			...
			if ( !empty($connections) && count($connections) > 0 ) {
				...
				foreach($connections as $index => $connection) {
					...
					if ( get_post_meta( $connection, 'wpcf-p-tag', true) != '' ) // true = no array
						$has_tag = true;
					if ( $has_guest == true ) {
						//$guests[] = types_render_field('p-e-guest-contact', array('item' => 'projection'));
	...
	if ( $atts['output'] == 'language' ) {
		...
		if ($has_guest == true) 
			$output .= sprintf('<span class="badge text-wrap color-black text-dark text-left text-uppercase normal" style="max-width: 7rem;">
		...
		if ($has_highlight == true) 
			$output .= sprintf('<i class="icon icon-sun"></i> Temps-fort ');
		if ($has_youngpublic == true) 
			$output .= sprintf('<i class="icon icon-young"></i> Parents-enfants ');
		if ($has_guest == true) 
			$output .= sprintf('<i class="icon icon-guest"></i> En présence de <span class="subline">%s</span> ',
			implode(',', $guests)
			);
		if ($has_tag == true) 
			$output .= sprintf('<span class="text-danger"><i class="icon icon-warning text-danger"></i> %s</span> ',
			print_r($tag, true)
			);
	} else {
...
function get_counts($taxonomy_name = 'section', $taxonomy_id = array(), $projections__in = array()) {
	...
	if ( !empty($projections__in) ) {
		...
		// Store count
		$counts['projections'] = count(array_diff( $projections__in, $projections_without_films));
		$counts['programs'] = count(array_diff( $projections__in, $belongs_film_projection_ids));
		...
		/**
		 * All projections not belongs to a film with query
		 */
		...
		$programs_args = array(
			...
			'order' 			=> 'DESC',
			'post_status' 		=> 'publish',
			'post__in'			=> $projections_without_films,
		...
		// Conditionnal ( rooms )
		if ( !empty($conditionnal_tax_query) && $taxonomy_name === 'room') 
			$programs_args['tax_query'][] = $conditionnal_tax_query;
		...
		// echo ('<b>Projection(s) WITHOUT films queried (programs)::count</b> ' . print_r(count($programs_results), true) . '<br/>');
		// echo ('<pre> Projection(s) WITHOUT films queried(programs)::' . print_r($programs_results, true) . '</pre>');
